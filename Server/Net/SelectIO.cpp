...
VOID SelectIO::AddListenSocket(CSocket *pListenSock) {
	...
	FD_SET(m_nListenSock, &m_oAllSet);
...
VOID SelectIO::AddNewConnection() {
	...
	++m_nSockets;
	cout << "accept new connection cur conns = " << m_nSockets << endl;
}
...
VOID SelectIO::DelConnection(Session *pSession) {
	...
	--m_nSockets;
	cout << "del connection cur conns = " << m_nSockets << endl;
}